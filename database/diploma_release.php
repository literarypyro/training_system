...
//$sql="select *,(select count(*) from diploma_release where trainee_id=class_list.trainee_id and training_program_id=class_list.training_events_id and type='diploma') as diploma_count from (class_list inner join training_events on class_list.training_events_id=training_events.id) inner join trainee_list on trainee_id=trainee_list.id where start_date between '".$latestDate." 00:00:00' and '".$currentYear."-12-31 23:23:59'";

$sql="select *,(select count(*) from diploma_release where trainee_id=class_list.trainee_id and training_program_id=class_list.training_events_id and type='diploma') as diploma_count from (class_list inner join training_events on class_list.training_events_id=training_events.id) inner join trainee_list on trainee_id=trainee_list.id";
$rs=$db->query($sql);
...
<form action="training_cert.php?a=dss&action=add" method="post">
	<table id='indexhere' align=center style='border: 1px solid gray' width=600>
	<th style="vertical-align:center;background-color:#ed5214;color:white;" colspan=3 ><h1>Issuance of Diploma Record</h1>
	</th>
	</table>
