...
if(isset($_POST['event_id'])){
	...
	$sql="insert into training_evaluation(event_id,trainer_id,comments,suggestions,lesson_learned,rater) values ";
	$sql.="('".$event_id."','".$trainer."',\"".$comments."\",\"".$suggestions."\",\"".$lesson_learned."\",\"".$rater."\")";
	$rs=$db->query($sql);
	
	$eval_id=$db->insert_id;
...
//		$db2=new mysqli("localhost","root","","evaluation");
		...
		for($i=1;$i<=$nm2;$i++){	
			$rating=$_POST["Category_".$i];

			$insert="insert into evaluation_category(evaluation_id,rating,category_parts) values ";
			$insert.="('".$eval_id."','".$rating."','".$i."')";
			$rs3=$db2->query($insert);
		}
...
<form action="training_evaluation.php?a=entry" method="post">
<table width=100%>
<tr>
<td align=center colspan=2>
	<table id='indexhere' align=center style='border: 1px solid gray' width=600>
	<th style="vertical-align:center;background-color:#ed5214;color:white;" colspan=3 ><h1>Submit Evaluation Data</h1>
	</th>
	</table>
...
<td>
	<table id='dynamicProgram' width=500 align=center style='border: 1px solid gray'>
			<tr>
				<th colspan=2  style="background-color: #00cc66;color: black;">Category</th>
				<th style="background-color: #00cc66;color: black;">Rating</th>
			</tr>	<?php
//		$db2=new mysqli("localhost","root","","evaluation");
		$db2=retrieveEvaluationDb();
		...
		for($a=0;$a<$nm2;$a++){
		$row2=$rs2->fetch_assoc();
			$sqlInner="select * from category where part='".$row2['id']."'";
			$rsInner=$db2->query($sqlInner);
			$nmInner=$rsInner->num_rows;

		?>	
		
		<tr>
		<th style="background-color:#ed5214;color:white;" rowspan=<?php echo $nmInner*1+1; ?> >
		<?php echo strtoupper($row2['category-part']); ?>
		</th>
		</tr>
		...
		<?php
			...
			for($b=0;$b<$nmInner;$b++){
				$rowInner=$rsInner->fetch_assoc();
			?>
				<tr>
				<td style="background-color: #00cc66;color: black;">
				<?php
				if($row2['id']==4){
				echo $rowInner['description']; 
				
				}
				else {
				?><b>
				<?php
				echo strtoupper($rowInner['title'])."<br>"; 
				?>
				</b>
				<?php
				echo $rowInner['description']; 
				...
				<td valign=center align=center style='background-color:white;color:black;'>
				<select name="<?php echo "Category_".$internalCount; ?>"  id="<?php echo "Category_".$internalCount; ?>">
				<?php
					$internalCount++;
					$sqlOption="select * from ranking order by id desc";
					$rsOption=$db2->query($sqlOption);
					$nmOption=$rsOption->num_rows;
					for($k=0;$k<$nmOption;$k++){
						$rowOption=$rsOption->fetch_assoc();
					?>
					<option value='<?php echo $rowOption['id']; ?>'><?php echo $rowOption['id']." - ".$rowOption['ranking']; ?></option>	
					<?php
...
<table style='border: 1px solid gray'>
	<tr>
	<th style="vertical-align:center;background-color:#ed5214;color:white;" colspan=2 >Others
	</th>
	</tr>
	<tr>
	<th style="background-color: #00cc66;color: black;">What was your most meaningful learning from this training?</th>
	<td style="background-color:white; color:black;">
	<textarea name="lesson_learned"></textarea>
	</td>
	
	</tr>
	<tr>
	<th style="background-color: #00cc66;color: black;">Suggestions to improve future trainings?</th>
	<td style="background-color:white; color:black;">
	...
	<tr>
	<th style="background-color: #00cc66;color: black;">Comments:</th>
	<td style="background-color:white; color:black;">
...
Name of Rater (Optional): <input type=text name='rater' size=50 />
<br>
<br>	
...
<table align=center>	
	...
	<input type=submit value='Submit Evaluation Data' />
	</td>
	</tr>
</table>
</td>
