...
//	$insert_db=new mysqli("localhost","root","","evaluation");
	...
	$sql="delete from plan_materials where event_id='".$_POST['selectedEvent']."'";
	...
	for($i=0;$i<count($objectives);$i++){
		if($objectives[$i]==""){
			...
			$sql="insert into plan_materials(event_id,material) values ('".$_POST['selectedEvent']."','".$objectives[$i]."')";
			$rs=$insert_db->query($sql);
...
//	$objectiveDb=new mysqli("localhost","root","","evaluation");
	...
	$objectiveSQL="select * from plan_materials where event_id='".$row2['id']."'";
	...
	for($m=0;$m<$objectiveNM;$m++){
		...
		echo "objectives['Event_".$row2['id']."'][objectiveCount['Event_".$row2['id']."']]=\"".$objectiveRow['material']."\";";
...
function fillDynamic(){
	...
	var objectiveGrid="";
	objectiveGrid+="<tr><th style='background-color: #00cc66;color: black;'>Materials/Facilities</th></tr>";
...
function finalizeSubmit(){
	...
	<th style="vertical-align:center;background-color:#ed5214;color:white;" colspan=3 ><h1>Modify Training Plan<br>Materials/Facilities</h1>
	</th>
	</table>
<table width=100% align=center >
	...
	<tr>
	<td align=center>
	<form id='objectives_form' name='objectives_form' action='training_plan_outline.php?a=materials' method='post'>
	<input type=hidden id='selectedEvent' name='selectedEvent' />
	<input type=hidden id='objectives_plan' name='objectives_plan' />
	<input type=button value='Submit Materials' onclick='finalizeSubmit();' />
	</form>
	</td>
	<td colspan=2>
	&nbsp;
