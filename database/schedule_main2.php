...
<?php
if(isset($_GET['edit'])){
	$sql="select * from event inner join schedule on event.id=schedule.event_id";
	$rs=$db2->query($sql);
	$nm=$rs->num_rows;
	
	if($nm>0){
		$row=$rs->fetch_assoc();
		$update="update training.training_events set training_program_id='".$row['program_id']."',batch_number='".$row['batch']."',start_date='".$row['date_from']."',end_date='".$row['date_to']."',no_of_days='".$row['no_of_days']."' where id='".$row['event_id']."'";

		$updateRS=$db2->query($update);
		

		$update="delete from training.class_list where training_events_id='".$row['event_id']."'";
		
		
		$updateRS=$db2->query($update);
		
		$update="delete from training.trainer_class where event_id='".$row['event_id']."'";
		$updateRS=$db2->query($update);
	
		
		$update="insert into training.trainer_class(event_id,trainer_id) (select event_id,trainer_id from trainer where event_id='".$row['event_id']."')";
		$updateRS=$db2->query($update);
		
		$update="insert into training.class_list(training_events_id,trainee_id,diploma_release,remarks) (select event_id,participant_id,'Unclaimed',remarks from participant where event_id='".$row['event_id']."')";
...
function submitDetails(){
	...
	if(message){
		window.open("training_event.php?a=AA&edit=Y","_self");
	}
...
<br>
<div align=center>
<a href='#' onclick='window.open("database/sked_add_event.php")'>Add/Change</a> 
</div>
<br>
...
?>
<div align=center>
<a href='#' onclick='window.open("database/sked_add_trainer.php")'>Add</a><!--enable only if Event has been added -->
</div>
<br>
<?php
}
...
?>
<div align=center>
<a href='#'  onclick='window.open("database/sked_add_period.php")' >Add/Change</a><!--enable only if Event has been added -->
</div>
<br>
<?php
}
...
?>
<div align=center>
<a href='#'  onclick='window.open("database/sked_add_participant.php")'>Add</a><!--enable only if Event has been added -->
</div>
<br>
<?php
}
