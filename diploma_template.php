...
?>
<?php
$_SESSION['training_page']="diploma_template.php";
?>
<?php
if(isset($_POST['content_type'])){
	if($_POST['content_type']==""){
		...
		$sql="select * from template where document_type='diploma' and content_type='".$_POST['content_type']."'";
		$rs=$db->query($sql);
		$nm=$rs->num_rows;
		...
		if($nm>0){
			...
			$sqlUpdate="update template set content='".$_POST['content']."',content_path='".$target_path."',content_order='".$content_order."' where document_type='diploma' and content_type='".$_POST['content_type']."'";
			$rsUpdate=$db->query($sqlUpdate);		
		}
		else {
			$sqlUpdate="insert into template(content,content_path,content_type,document_type,content_order) values ('".$_POST['content']."','".$target_path."','".$_POST['content_type']."','diploma','".$content_order."')";
			$rsUpdate=$db->query($sqlUpdate);		
		}
		echo "Template has been updated.";
			$timestamp=date("Y-m-d His");
			echo "<script language='javascript'>";
			echo "window.open('print_outline2.php?diplomaId=SAMPLE&traineeId=SAMPLE&timestamp=".$timestamp."');";

			echo "</script>";
			$message="<font style='background-color:red;color:white;'>Template has been updated.</font>";			
			$message.="<br><br><font style='background-color:red;color:white;'>Click <a style='text-decoration:none' href='printout/Diploma ".$timestamp.".pptx'>here</a> for the sample</font><br>";	
			...
			echo "</script>";
			$message="";			
			$message.="<font style='background-color:red;color:white;'>Click <a style='text-decoration:none' href='printout/Diploma ".$timestamp.".pptx'>here</a> for the Existing Template</font><br>";	
...
//$db=new mysqli("localhost","root","","training");
$db=retrieveTrainingDb();
$sql="select * from template where document_type='diploma'";
$rs=$db->query($sql);
$nm=$rs->num_rows;
for($i=0;$i<$nm;$i++){
	$row=$rs->fetch_assoc();
...
<tr>
	...
	<th style="vertical-align:center;background-color:#ed5214;color:white;" colspan=3 ><h1>Diploma Template</h1>
	</th>
	</table>
	
	<br>
	<?php echo $message; ?>	
	<form enctype="multipart/form-data" action="diploma_template.php" method="post">
	...
	<table>
		...
		<td style="background-color:white; color:black;">
			...
			<option value='footer_name2'>Right Footer Name</option> 
			<option value='footer_position2'>Right Footer Position</option> 
			<option value='footer_location2'>Right Footer Location</option> 
			<option value='background_image'>Background Image</option> 
			<option value='venue'>Training Venue</option> 
