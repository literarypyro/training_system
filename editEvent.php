...
if(isset($_GET['retrieveID'])){
		...
		if($nm>0){
			...
			$update="insert into participant(participant_id,first_name,middle_name,last_name,designation,department,event_id) (select traineeId,firstName,midInitial,lastName,designation,department,training_event_id from training.class_instance where training_event_id='".$_GET['retrieveID']."')";
			$updateRS=$db2->query($update);
		...
		echo "window.opener.location='training_event.php?a=AA';";
		echo "self.close();";
		echo "</script>";		
