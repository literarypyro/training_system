...
//	$db2=new mysqli("localhost","root","","evaluation");
	...
	for($m=0;$m<count($trainee);$m++){
		...
		if($nm2<$requiredAttendance){
			...
			$sqlRemarks="select * from remarks where event_id='".$event_id."' and trainee_id='".$trainee[$m]['ID']."'";
			$rsRemarks=$db2->query($sqlRemarks);
			$rowRemarks=$rsRemarks->fetch_assoc();
			$incomplete[$incompleteCount]['remarks']=$rowRemarks['remarks'];
	...
	$newFilename="printout/Report of Attendance ".$datePrintout.".xls";
	...
	addContent(setRange("J".$rowCount,"L".($rowCount*1+1)),$excel,"REMARKS","true",$ExWs);		
	$excel->getActiveSheet()->getStyle("J".$rowCount.":L".($rowCount*1+1))->getFont()->setBold(true);
	$excel->getActiveSheet()->getStyle("J".$rowCount.":L".($rowCount*1+1))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
	$excel->getActiveSheet()->getStyle("J".$rowCount.":L".($rowCount*1+1))->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
	$styleArray = array(
		'borders' => array(
			'outline' => array(
				'style' => PHPExcel_Style_Border::BORDER_THIN,
	...
	$excel->getActiveSheet()->getStyle("J".$rowCount.":L".($rowCount*1+1))->applyFromArray($styleArray);
	...
	$excel->getActiveSheet()->getStyle("J".$rowCount.":L".$rowCount)->getBorders()->getLeft()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
	$excel->getActiveSheet()->getStyle("J".$rowCount.":L".$rowCount)->getBorders()->getRight()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
	...
	addContent(setRange("J".$rowCount,"L".($rowCount*1)),$excel,"","true",$ExWs);		
	$excel->getActiveSheet()->getStyle("J".$rowCount.":L".$rowCount)->getBorders()->getLeft()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
	$excel->getActiveSheet()->getStyle("J".$rowCount.":L".$rowCount)->getBorders()->getRight()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
	$excel->getActiveSheet()->getStyle("J".$rowCount.":L".$rowCount)->getBorders()->getBottom()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
	...
	$excel->getActiveSheet()->getStyle("J".$rowCount.":L".$rowCount)->getBorders()->getLeft()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
	$excel->getActiveSheet()->getStyle("J".$rowCount.":L".$rowCount)->getBorders()->getRight()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
	$excel->getActiveSheet()->getStyle("J".$rowCount.":L".$rowCount)->getBorders()->getTop()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
	...
	for($i=0;$i<count($incomplete);$i++){
		...
		addContent(setRange("J".$rowCount,"L".$rowCount),$excel,$incomplete[$i]['remarks'],"true",$ExWs);			
		$excel->getActiveSheet()->getStyle("J".$rowCount.":L".$rowCount)->getBorders()->getLeft()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
		$excel->getActiveSheet()->getStyle("J".$rowCount.":L".$rowCount)->getBorders()->getRight()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
	...
	xmlHttp.onreadystatechange=function()
	{
		if (xmlHttp.readyState==4 && xmlHttp.status==200)
		{
			...
			if(traineeHTML=="None available"){
				...
				document.getElementById('training_id').value=traineeTerms[0];
				document.getElementById('period').value="From "+traineeTerms[3]+" to "+traineeTerms[4];
				selectedTrainingID=traineeTerms[0];
		
				
				document.getElementById('event_id').value=traineeTerms[0];
...
||-->
	...
	<form action='memo_generate.php?generate=complete_report2' method='post' >	
	<table align=center>	
	<tr>
